...
class JndiPropertiesProvider extends PropertiesProvider{
  ...
  private def translateKey(key: ConfigurationKey) = key match {
    ...
    case MailSmtpFrom => "ANTARCTICLE_SMTP_FROM"
    case MailSmtpUser => "ANTARCTICLE_SMTP_USER"
...
class TypesafeConfigPropertiesProvider(config: Config) extends PropertiesProvider {
  ...
  private def translateKey(key: ConfigurationKey) = key match {
    ...
    case MailSmtpFrom => "mail.smtp.from"
    case MailSmtpUser => "mail.smtp.user"
    ...
    case PoulpeUsername => "security.authentication.poulpe.username"
    case PoulpeSecret => "security.authentication.poulpe.secret"
