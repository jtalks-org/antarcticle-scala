...
case class UserRecord(id: Option[Int], username: String, password: String, admin: Boolean = false,
...
trait UsersSchemaComponent {
  ...
  class Users(tag: Tag) extends Table[UserRecord](tag, "users") {
    ...
    // projections
    def * = (id.?, username, password, admin, firstName.?, lastName.?, rememberToken.?) <> (UserRecord.tupled, UserRecord.unapply)
  }
