 ...
 */
private[security] case class UserInfo(username: String, password: String, firstName: Option[String],
                                      lastName: Option[String], active: Boolean)

private[security] object UserInfoImplicitConversions {
  implicit def UserRecordOption2UserInfoOption(record: UserRecord) =
    new UserInfo(record.username, record.password, record.firstName, record.lastName, record.active)
}
