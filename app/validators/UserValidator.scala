  ...
  override def validate(user: User): ValidationNel[String, User] = {
    ...
    def checkUsername = {
      if (user.username.trim.length > USERNAME_MAX_LENGTH || user.username.trim.isEmpty)
        s"Username length must be between 1 and $USERNAME_MAX_LENGTH characters".failureNel
      else
        user.successNel
    }
    ...
    def checkPassword = {
      if (user.password.trim.length > PASSWORD_MAX_LENGTH  || user.password.trim.isEmpty)
        s"Password length must be between 1 and $PASSWORD_MAX_LENGTH characters".failureNel
      else
        user.successNel
    }
    ...
    def checkEmail = {
      if (user.email.trim.isEmpty || !user.email.matches(EMAIL_FORMAT))
        "An email format should be like mail@mail.ru".failureNel
      else if (user.email.trim.length >= EMAIL_MAX_LENGTH)
        s"Email field should not contain more than $EMAIL_MAX_LENGTH symbols.".failureNel
      else
        user.successNel
    }
