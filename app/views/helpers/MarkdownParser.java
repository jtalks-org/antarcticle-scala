 ...
 * todo: rewrite it in scala and remove unnecessary syntax extensions
 */
@SuppressWarnings({"InfiniteRecursion"})
public class MarkdownParser extends BaseParser<Object> implements Extensions {

  protected static final char CROSSED_OUT = '\uffff';

  protected final long maxParsingTimeInMillis = PegDownProcessor.DEFAULT_MAX_PARSING_TIME;
  ...
  public Rule Block() {
    return Sequence(
            ...
            FirstOf(new ArrayBuilder<Rule>()
                    .addNonNulls(FencedCodeBlock())
                    .add(BlockQuote(), Verbatim())
                    .addNonNulls(Abbreviation())
  ...
  public Rule HorizontalRule(char c) {
    return Sequence(c, c, c, ZeroOrMore(c));
  }
  ...
  public Rule NonLinkInline() {
    return FirstOf(new ArrayBuilder<Rule>()
            .add(Str(), Endline(), UlOrStarLine(), Space(), StrongOrEmph(), Image(), Code(), InlineHtml(),
                    Entity(), EscapedChar())
            .addNonNulls(SingleQuoted(), DoubleQuoted(), DoubleAngleQuoted())
            .addNonNulls(Smarts())
            .addNonNulls(Strike())
            .add(Symbol())
            .get()
  ...
  public Rule BlockCode(Rule ticks) {
    return Sequence(
            ticks, Sp(),
            ZeroOrMore(TestNot(Newline()), ANY), // GFM code type identifier
            push(match()),
            ...
            push(new VerbatimNode(match().trim(), popAsString())),
            Sp(), ticks
    );
  }

  public Rule InlineCode(Rule ticks) {
    return Sequence(
            ticks,
            OneOrMore(
                    FirstOf(
  ...
  public RootNode parseInternal(char[] source) {
    ...
    if (result.hasErrors()) {
      throw new RuntimeException("Error during markdown parsing:\n--- ParseErrors ---\n" +  printParseErrors(result));
    }
  ...
  ParsingResult<Node> parseToParsingResult(char[] source) {
    ...
    return new ReportingParseRunner<Node>(Root()).run(source);
  }
