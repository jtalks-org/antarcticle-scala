  ...
  def validate(article: Article): ValidationNel[String, Article] = {
    def checkContentLength = {
      if (article.content.length > MAX_CONTENT_LENGTH)
        s"Article text should not exceed $MAX_CONTENT_LENGTH characters".failureNel else article.successNel
    }
    def checkTitleLength = {
      if (article.title.trim.isEmpty) "Title should not be blank".failureNel
      else if (article.title.length > MAX_TITLE_LENGTH)
        s"Title should not exceed $MAX_TITLE_LENGTH characters".failureNel else article.successNel
    }
    def checkTagsCount = if (article.tags.distinct.length > MAX_TAGS_COUNT)
      s"At most $MAX_TAGS_COUNT different tags are allowed".failureNel else article.successNel
